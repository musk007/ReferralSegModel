...
class XDecoder_Trainer(DefaultTrainer):
    ...
    def create_optimizer_and_scheduler(self):
        ...
        lora_cfg = self.opt['SOLVER'].get('LORA', {})
        use_lora = lora_cfg.get('ENABLED', False)
        if use_lora:
            # LoRA mode: freeze all params, then unfreeze only lora_A and lora_B
            ...
            logger.info("LoRA mode: only lora_A and lora_B parameters are trainable")
        ...
        # logger.info(f"[GLOBAL] Total: {global_total}")
        # logger.info(f"[GLOBAL] Trainable: {global_trainable}")
        # logger.info(f"[GLOBAL] Frozen: {global_frozen}")
        # import sys; sys.exit()
        # --------------------------------------------------------------
        ...
        for module_name in self.model_names:
            # Build a fresh plain-dict copy so yacs CfgNode converts it
            # recursively (type(v) is dict check), and explicitly include
            # RESCALE_INTERVAL which newer detectron2 requires.
            solver_dict = {**cfg_solver, 'RESCALE_INTERVAL': cfg_solver.get('RESCALE_INTERVAL', False)}
            scheduler_cfg = CfgNode({'SOLVER': solver_dict})
