...
class GeneralizedSEEM(nn.Module):
    ...
    def forward_seg(self, batched_inputs):
        # print("*"*20)
        # print("text in forward_seg() ....")
        # print(batched_inputs[0].keys())
        # print(batched_inputs[0]['file_name'])
        # print(batched_inputs[0]['groundings']["texts"])
        # print("*"*20)
        # import sys; sys.exit()
    ...
    def evaluate_demo(self, batched_inputs):
        """Text-grounded inference compatible with interactive_infer_image.
        Same interface as seem_model_demo.evaluate_demo for loading fine-tuned v1 checkpoints."""
        ...
        if 'text' in batched_inputs[0]:
            gtext = self.sem_seg_head.predictor.lang_encoder.get_text_token_embeddings(
                batched_inputs[0]['text'], name='grounding', token=False, norm=False
            )
        ...
        if 'pred_logits' in outputs:
            results['pred_logits'] = outputs['pred_logits']
        return results, images.tensor.shape, extra
