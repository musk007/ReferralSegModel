...
OUTPUT_DIR="${OUTPUT_DIR:-${DATASETS_DIR}/output/QueryFeat_ClassHead_TxtProj_SegHead_Pubmed}"
...
mpirun -n ${NUM_GPUS} --oversubscribe --bind-to none python entry.py train \
    --conf_files configs/biomed_seg_lang_v1.yaml biomed_seg_lang_v1_histopath_query.yaml \
    "${CONFIG_OVERRIDES_ARGS[@]}" \
    --overrides \
    SAVE_DIR "${OUTPUT_DIR}" \
    FP16 True \
